...
def get_model():
	...
	embedded_layer = Embedding(input_dim=2000000,output_dim=6,input_length=40,mask_zero=False)(input1)
	flatten_layer = Flatten()(embedded_layer)
	hidden = Dense(64,activation='relu')(flatten_layer)
	hidden1 = Dense(32,activation='relu')(hidden)
	hidden2 = Dense(16,activation='relu')(hidden1)
	out = Dense(2,activation = 'softmax',kernel_initializer='uniform')(hidden2)#,W_regularizer=regularizers.l2(0.002)
...
def mygenerator(batch_size,data_size):
	accumulation=0
	f = open('/home/joseph/文件/prune_data/train_out100.txt')
	while True:
...
if __name__ == "__main__":
	...
	history = model.fit_generator(mygenerator(batch_size,data_size),steps_per_epoch=int(data_size/batch_size)+1,epochs=1000,verbose=1,validation_data=(padded_x_test,y_test_ca),class_weight={0:1, 1:1800},callbacks=[roc_callback(testing_data=(padded_x_test,y_test_ca)),early_stopping])
	print(cw)
